...
// Barter system: Teacher must provide barterSkill to learn from learner
router.put('/respond/:id', auth, async (req, res) => {
  const { response, scheduledTime, durationHours, numberOfWeeks, daysOfWeek, timeOfDay, barterSkill, barterSchedule } = req.body; 
  ...
  try {
        ...
        try {
          ...
          if (teacher && teacher.calendarProvider === session.calendarProvider) {
            await calendarService.deleteEvent(
              ...
              session.calendarProvider
            );
          }
        } catch (calendarErr) {
          console.error('Error deleting calendar event:', calendarErr.message);
    ...
    if (response === 'confirmed') {
      // Barter system: Teacher must select a skill to learn from the learner
      // EXCEPT if this is a reverse barter request (barterSessionId exists), then skill was already selected
      const isReverseBarterRequest = !!session.barterSessionId;
      
      if (!isReverseBarterRequest && !barterSkill) {
        return res.status(400).json({ 
          msg: 'This is a barter system. Please select a skill you want to learn from the learner.' 
        });
      }
      
      // For reverse barter requests, don't create another barter session
      let barterSession = null;
      if (isReverseBarterRequest) {
        // This is a reverse barter request - skill was already selected, just proceed with scheduling
      } else if (barterSkill) {
        // Create reverse barter request (learner becomes teacher, teacher becomes learner)
        try {
          barterSession = new Session({
            learner: session.teacher._id, // Original teacher becomes learner
            teacher: session.learner._id, // Original learner becomes teacher
            skill: barterSkill,
            status: 'pending', // Will be confirmed when original learner schedules it
            barterSessionId: session._id, // Link to original session
          });
          await barterSession.save();
          
          // Link original session to barter session
          session.barterSessionId = barterSession._id;
        } catch (barterErr) {
          console.error('Error creating barter session:', barterErr.message);
          return res.status(500).json({ msg: 'Failed to create barter session request' });
        }
      }
      ...
      if (numberOfWeeks && daysOfWeek && timeOfDay) {
        ...
        // Create calendar event if teacher has calendar connected
        try {
          const teacher = await User.findById(session.teacher);
          if (teacher && teacher.calendarProvider) {
            const calendarResult = await calendarService.createEvent(
              session.teacher,
              session,
              teacher.calendarProvider
            );
            session.calendarEventId = calendarResult.eventId;
            session.calendarProvider = teacher.calendarProvider;
            session.calendarLink = calendarResult.htmlLink || calendarResult.webLink;
          }
        } catch (calendarErr) {
          console.error('Error creating calendar event:', calendarErr.message);
        ...
        for (let i = 1; i < sessionDates.length; i++) {
          ...
          try {
            ...
            if (teacher && teacher.calendarProvider) {
              ...
              newSession.calendarEventId = calendarResult.eventId;
              newSession.calendarProvider = teacher.calendarProvider;
              newSession.calendarLink = calendarResult.htmlLink || calendarResult.webLink;
            }
          } catch (calendarErr) {
            console.error('Error creating calendar event:', calendarErr.message);
          }
...
router.post('/create-meeting/:id', auth, async (req, res) => {
  try {
    ...
    try {
      ...
      // Update calendar event with meeting link if it exists
      if (session.calendarEventId && session.calendarProvider) {
        try {
          const teacher = await User.findById(session.teacher);
          if (teacher && teacher.calendarProvider === session.calendarProvider) {
...
router.put('/reschedule/:id', auth, async (req, res) => {
  ...
  try {
    ...
    // Both teacher and learner can reschedule
    if (
      session.teacher._id.toString() !== req.user.id &&
      session.learner._id.toString() !== req.user.id
    ) {
      return res.status(401).json({ msg: 'Only the teacher or learner can reschedule sessions' });
    }
    ...
    if (session.calendarEventId && session.calendarProvider) {
      ...
      // Create calendar event if teacher has calendar connected but event doesn't exist
      try {
        const teacher = await User.findById(session.teacher);
        if (teacher && teacher.calendarProvider) {
          const calendarResult = await calendarService.createEvent(
...
router.put('/cancel/:id', auth, async (req, res) => {
  try {
    ...
    // Only teacher or learner can cancel
    if (
      session.teacher._id.toString() !== req.user.id &&
      session.learner._id.toString() !== req.user.id
    ) {
    ...
    // If session was previously completed, reduce the hours
    if (wasCompleted && hours > 0) {
