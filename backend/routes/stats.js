...
router.get('/skills', auth, async (req, res) => {
  try {
    // Get all completed learning sessions
    const learningSessions = await Session.find({ 
      learner: req.user.id, 
      status: 'completed'
    });
    ...
    const skillProgress = learningSessions.reduce((acc, session) => {
      ...
      if (!acc[skill]) {
        acc[skill] = {
          skill,
          hoursCompleted: 0,
          totalHours: 0, // We'll set a target, e.g., 10 hours per skill
        };
      }
      acc[skill].hoursCompleted += hours;
      acc[skill].totalHours = Math.max(acc[skill].totalHours, 10); // Default target of 10 hours
    ...
    const skillsArray = Object.values(skillProgress).map(skill => ({
      ...skill,
      progress: Math.min((skill.hoursCompleted / skill.totalHours) * 100, 100),
    }));
